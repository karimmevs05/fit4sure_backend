...
//
// Restored from the pre-Operations-Hub taskManagementAuto.js, which this
// route replaces. Coexists with the new generic `tasks` table via
// source_type = 'production_task' (see adminTasks.js) rather than being
// folded into it -- this data model (live-computed schedule + status/issue
// tables keyed by week_start/day/plate_id) is materially different from a
// persisted task row and isn't a duplicate task system.
// ============================================================================
...
router.get('/weeks-with-plates', requireAuth, requireRole('admin'), async (req, res) => {
  try {
...
router.post('/auto-generate-plan', requireAuth, requireRole('admin'), async (req, res) => {
  try {
...
// ============================================================================
router.post('/task-status', requireAuth, requireRole('admin'), async (req, res) => {
  try {
...
// ============================================================================
router.get('/issues', requireAuth, requireRole('admin'), async (req, res) => {
  try {
...
router.put('/issues/:id/resolve', requireAuth, requireRole('admin'), async (req, res) => {
  try {
    const result = await pool.query(
